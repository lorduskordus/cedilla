...
impl Content {
    ...
    pub fn push_str(&mut self, markdown: &str) {
        ...
        for (item, source, broken_links) in parse_with(&mut self.state, &leftover) {
...
impl Item {
        ...
        Renderer: cosmic::iced_core::text::Renderer<Font = Font> + 'a,
    {
        ...
        match self {
            ...
            Item::Paragraph(paragraph) => rich_text(paragraph.spans(style)).size(text_size).into(),
            ...
            {
                {
                    highlighter = Some({
                        let mut highlighter = state
                            ...
                            .highlighter
                            .take()
                            .filter(|highlighter| highlighter.language == _language.as_ref())
        ...
        pulldown_cmark::Event::Text(text) if !metadata && !table => {
            if let Some(highlighter) = &mut highlighter {
                for line in text.lines() {
                    code.push(Text::new(highlighter.highlight_line(line).to_vec()));
                }
